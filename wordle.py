...
def safe_remove(l, elem):
    if elem in l:
        l.remove(elem)


def get_allowed_letters(attempts):
    n_attempts = int(len(attempts) // 2)

    # Derive allowed letters in every position
    allowed_letters = [copy.deepcopy(all_letters) for _ in range(5)]

    for i in range(n_attempts):
        attempt, result = attempts[i*2], attempts[i*2+1]

        # Iterate letters in an attempt
        for j in range(5):
            letter = attempt[j]
            # print(j, letter)

            # Black letters are disallowed in every position
            if result[j] == 'b':
                # print(f'Remove black {letter} from all positions')
                [safe_remove(allowed_letters[k], letter) for k in range(5)]

            # Yellow letters are disallowed in the position they occur
            if result[j] == 'y':
                # print(f'Remove yellow {letter} from position {j}')
                safe_remove(allowed_letters[j], letter)

            # Green letters are the only ones allowed in the position they occur
            if result[j] == 'g':
                # print(f'Remove all but green {letter} from position {j}')
                allowed_letters[j] = [letter]

    return allowed_letters


def green_union(attempts):
    n_attempts = int(len(attempts) // 2)
    union = ['_' for _ in range(5)]
    for i in range(n_attempts):
        attempt, result = attempts[i*2], attempts[i*2+1]
        for j in range(5):
            if result[j] == 'g':
...
def get_known_occurrences(attempts):
    union = green_union(attempts)
    known_occurrences = {}
    for letter in set(union):
        if letter != '_':
            count = union.count(letter)
            known_occurrences[letter] = count
...
def get_minimum_occurrences(attempts):
    # Minimum occurrences are defined by green and yellow letters.
    ...
    for i in range(n_attempts):
        ...
        for letter in set(attempt):
            ...
            if letter not in minimum_occurrences or count > minimum_occurrences[letter]:
                minimum_occurrences[letter] = count
    return minimum_occurrences


def filter_words(allowed_letters, minimum_occurrences):
    # Iterate words in vocabulary
    matches = []
    for word in allowed:
        match = 1
...
def get_possible_words(attempts):
    ...
    minimum_occurrences = get_minimum_occurrences(attempts)
